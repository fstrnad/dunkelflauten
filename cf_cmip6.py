...
for tr_idx, (tr_str, cf_dict_cmip) in enumerate(cf_tr_dict.items()):
    ...
    for idx, (sname, source) in enumerate(cf_dict_cmip.items()):
        if sname != 'wind' and sname != 'all':
            ...
            vmin = -0.03 if sname == 'solar' else -0.2
...
def get_gcm_average(ssp, cf_dicts,
    ...
    cf_dicts = cf_dicts
    time_ranges = cf_dicts[gcms[0]][ssp].keys()
    source_cf_gcm_dict = {}
    for source in source_types:
        ...
        for tr_str in time_ranges:
            ...
            for gcm in gcms:
                if cf_dicts[gcm][ssp]:
                    tr_arr.append(cf_dicts[gcm][ssp][tr_str][source]['cf'])
            av_cf_gcm_tr_dict[tr_str] = xr.concat(
                tr_arr, dim='gcm').mean(dim='gcm')
            std_cf_gcm_tr_dict[tr_str] = xr.concat(
                tr_arr, dim='gcm').std(dim='gcm')
        source_cf_gcm_dict[source] = (av_cf_gcm_tr_dict, std_cf_gcm_tr_dict)
...
for idx, (sname, source) in enumerate(source_cf_gcm_dict=get_gcm_average(ssp=ssp,
                                                                         cf_dicts=gcm_ssp_cf_dict,).items()):
    source_avs, source_stds = source
    for tr_idx, (tr_str, source_av) in enumerate(source_avs.items()):
        if sname != 'wind' and sname != 'all':
            sd_tr, ed_tr = tu.get_time_range(
                gcm_ssp_cf_dict[gcm][ssp][tr_str]['offwind']['ts'], asstr=True)
            ...
            vmin = -0.01 if sname == 'solar' else -0.3
            gplt.plot_map(cap_fac - cf_0,
                          ax=im_diff['ax'][(tr_idx)*ncols + idx],
                          title=sname if tr_idx == 0 else None,
                          vertical_title=vertical_title if idx == 0 else None,
                          ...
                          cmap='cmo.tarn_r',
                          centercolor='white',
                          label=f'Diff. CF ERA5 (1980-2015)' if tr_idx == nrows - 1
                          else None,
                          plot_borders=True)

savepath_diff = f"{config['plot_dir']}/capacities_cmip6/cf_types_diff_ensemble_mean_{ssp}_{tr_str}.png"
gplt.save_fig(savepath_diff, fig=im_diff['fig'])
# %%
...
source_im_dict = {
    'onwind': {'ls': 'dashed', 'y_range': (0.15, 0.35)},
    'offwind': {'ls': 'dashdot', 'y_range': (0.35, 0.59)},
    'solar': {'ls': 'dotted', 'y_range': (0.1, 0.12)},
    'all': {'ls': 'solid', 'y_range': (0.15, 0.25)},
}
...
ref_ts_cmip = gcm_ssp_cf_dict[all_gcms[1]][ssp][tr_ref][sources[0]]['ts']
...
for idx, source in enumerate(sources):
    for idx_gcm, gcm in enumerate(all_gcms):
        source_cf_gcm_dict = get_gcm_average(ssp=ssp,
                                             cf_dicts=gcm_ssp_cf_dict,)
        if gcm == 'ERA5':
            cf_tr_dict = {tr_ref: cf_dict_era5}
        else:
            ssp_cf_dict = gcm_ssp_cf_dict[gcm]
            cf_tr_dict = ssp_cf_dict[ssp]
        if not cf_tr_dict:
        ...
        for tr_str, cf_dict_cmip in cf_tr_dict.items():
            ...
            da = cap['ts_uncorr'] if gcm == 'ERA5' and source == 'offwind' else cap['ts']
            if gcm == 'ERA5':
    ...
    vmin = -0.01 if source == 'solar' else -0.3
...
# %%
# Plot the ensemble mean of the capacity factors for Germany


def get_gcm_ts_average(ssp, cf_dicts,
                       source_types=['onwind', 'offwind',
                                     'solar', 'all']):
    ...
    for source in source_types:
        ...
        for tr_str in time_ranges:
            ...
            for gcm in gcms:
                if cf_dicts[gcm][ssp]:
                    tr_arr.append(cf_dicts[gcm][ssp][tr_str][source]['ts'])
            av_cf_gcm_tr_dict[tr_str] = xr.concat(
                tr_arr, dim='gcm').mean(dim='gcm')
            std_cf_gcm_tr_dict[tr_str] = xr.concat(
                tr_arr, dim='gcm').std(dim='gcm')
...
av_ts = get_gcm_ts_average(ssp='ssp585',
                           cf_dicts=gcm_ssp_cf_dict,)
